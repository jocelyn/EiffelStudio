...
feature -- Setting
	...
	set_feature_name_id (a_id: INTEGER; alias_id: INTEGER)
		...
		do
			...
			alias_name_ids := Void
			if alias_id /= 0then
				add_alias_id (alias_id)
			end
		ensure
			feature_name_id_set: feature_name_id = a_id
			alias_name_id_set: alias_id /= 0 implies attached alias_name_ids as lst and then lst.first = alias_id
		end
...
feature -- Undefinition
	...
	new_deferred: like new_deferred_anchor
		...
		do
			...
			if attached alias_name_ids as l_ids and then not l_ids.is_empty then
				across
					l_ids as ic
				loop
					Result.add_alias_id (ic.item)
				end
			end
...
feature -- Replication
	...
	transfer_to (other: FEATURE_I)
		...
		do
			...
			if attached alias_name_ids as l_ids then
				across
					l_ids as ic
				loop
					other.add_alias_id (ic.item)
				end
			end
	...
	transfer_from (other: FEATURE_I)
		...
		do
			...
			alias_name_ids := other.alias_name_ids -- FIXME: twin list of alias ids?
